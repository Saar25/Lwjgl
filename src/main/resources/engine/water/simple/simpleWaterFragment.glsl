...
float calcFresnelFactor(vec3 toCamera) {
    float fresnelFactor = toCamera.y; //dot(UP, toCamera);
...
vec3 applyFresnelEffect(vec3 reflection, vec3 refraction) {
    ...
    float fresnel = calcFresnelFactor(toCamera);
...
vec3 calculateRefractionColour(vec2 ndc) {
    if (hasTexture(1)) {
        vec2 refractionCoord = clamp(ndc, 0.001, 0.999);
