...
import engine.postprocessing.deferredRendering.DeferredRendering;
...
public class DeferredRenderingShowcase extends SimpleApplication {

    public static void main(String[] args) {
        try {
            Application game = new DeferredRenderingShowcase();
            GameEngine gameEngine = new GameEngine("DeferredRenderingShowcase", 1200, 741, game);
            gameEngine.init();
            gameEngine.start();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void onInit(Window window, RenderManager renderer, Keyboard keyboard, Mouse mouse) throws Exception {
        // Initialize the game
        keyboard.onKeyPress('T').perform(e -> DeferredRendering.inc());

        renderer.getPostProcessing().add(new DeferredRendering(camera));
